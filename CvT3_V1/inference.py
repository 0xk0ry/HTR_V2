...
def predict_single_image(model, decoder, image_path, device, decode_mode='both', beam_width=100):
    ...
    with torch.no_grad():
        ...
        if decode_mode in ('beam', 'both'):
            beam_result = decoder.beam_search_decode(
                pred_logits, beam_width=beam_width)
            result['beam_search'] = beam_result
        result['confidence'] = torch.softmax(
            pred_logits, dim=-1).max(dim=-1)[0].mean().item()
        return result
    ...
    for image_file in image_files:
        ...
        try:
            ...
            if ground_truth is not None:
                if 'greedy' in prediction:
                    cer_greedy = calculate_cer(
                        prediction['greedy'], ground_truth)
                    wer_greedy = calculate_wer(
                        prediction['greedy'], ground_truth)
                ...
                if 'greedy' in prediction and 'beam_search' in prediction:
                    print(
                        f"  CER (Greedy/Beam): {result.get('cer_greedy', 0):.3f} / {result.get('cer_beam_search', 0):.3f}")
                    print(
                        f"  WER (Greedy/Beam): {result.get('wer_greedy', 0):.3f} / {result.get('wer_beam_search', 0):.3f}")
                    ...
                    print(
                        f"  CER (Beam): {result.get('cer_beam_search', 0):.3f}")
                    print(
                        f"  WER (Beam): {result.get('wer_beam_search', 0):.3f}")
...
def main():
    ...
    if args.image:
        ...
        prediction = predict_single_image(
            model, decoder, args.image, device, decode_mode=args.decode_mode, beam_width=args.beam_width)

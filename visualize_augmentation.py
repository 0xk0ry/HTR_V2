...
from model.HTR_3Stage import DEFAULT_VOCAB
...
def create_augmentation_grid():
    ...
    aug_types = ["original", "affine", "erosion", "dilation", "noise", "elastic"]
    aug_names = ["Original", "Affine Transform", "Erosion", "Dilation", "Gaussian Noise", "Elastic Distortion"]
    
    # Create vertical figure (6 rows, 1 column)
    fig, axes = plt.subplots(len(aug_types), 1, figsize=(12, 18))
    fig.suptitle(f'Data Augmentation Examples: "{text}"', fontsize=16, fontweight='bold')
    ...
    for aug_idx, (aug_type, aug_name) in enumerate(zip(aug_types, aug_names)):
        ...
        try:
            ...
            if aug_image.size[1] != 64:
                aug_image = resize_to_target_height(aug_image, target_height=64)
...
def create_combined_augmentation_example():
    ...
    fig.suptitle(f'Combined Augmentation Examples: "{text}"', fontsize=16, fontweight='bold')
    ...
    for idx, (combo_name, aug_list) in enumerate(combinations):
        ...
        for aug_type in aug_list:
            try:
                ...
                print(f"Error in combination {combo_name} with {aug_type}: {e}")
                continue
